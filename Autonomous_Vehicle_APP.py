...
class CombinedYOLODetector:
    ...
    def find_available_cameras(self, max_to_test=5):
        """
        Detect available cameras
        
        Args:
            max_to_test (int): Maximum number of cameras to test
    ...
    def process_webcam(self, conf_threshold=0.25, iou_threshold=0.45, camera_index=0):
        ...
        if not cap or not cap.isOpened():
            st.error(f"Failed to open camera at index {camera_index} after {max_attempts} attempts.")
            st.error("Please check:")
            st.error("- Camera is properly connected")
            st.error("- No other application is using the camera")
        ...
        try:
            ...
            st.error(f"An error occurred during webcam processing: {str(e)}")
...
def main():
    ...
    if any(selected_models.values()):
        ...
        if input_type == "Image":
            ...
            # Detect available cameras
            available_cams = detector.find_available_cameras()
            
            if available_cams:
                st.success(f"Detected available cameras at indices: {available_cams}")
                camera_index = st.selectbox("Select Camera", available_cams)
                
                if st.button("Start Webcam"):
                    with st.spinner("Starting webcam..."):
                        detector.process_webcam(conf_threshold, iou_threshold, camera_index)
            else:
                st.warning("No cameras detected automatically. You can try manual mode.")
                camera_index = st.number_input("Enter camera index manually", min_value=0, max_value=10, value=0)
                
                if st.button("Start Webcam (Manual Mode)"):
                    with st.spinner("Attempting to start webcam..."):
                        detector.process_webcam(conf_threshold, iou_threshold, camera_index)
    else:
        st.warning("Please select at least one model from the sidebar to begin.")
    ...
    with st.expander("Instructions & Information"):
        ...
        ### Webcam Tips:
        - If automatic detection fails, try manual mode
        - Common camera indices are 0 (built-in) and 1 (external)
        - Close other applications that might be using the camera
        - On Linux, you might need to install v4l-utils: `sudo apt install v4l-utils`
